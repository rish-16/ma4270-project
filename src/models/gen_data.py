...
def generate_square_data(train_data_path: Path, test_data_path: Path, train_num_steps: int, test_num_steps: int, seq_len: int, interval: float = 0.1) -> None:
    ...
    test_start, test_end = train_end - context_len * \
        interval, train_end + test_num_steps * interval

    train_x = np.linspace(train_start, train_end,
                          train_num_steps, endpoint=False)
    train_y = signal.square(train_x)

    test_x = np.linspace(test_start, test_end,
                         test_num_steps + context_len, endpoint=False)
